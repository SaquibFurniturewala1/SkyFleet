...
function checkLoginInputs() {
  ...
  if (loginEmail.value === '' || loginEmail.value === null) {
    loginEmail.parentElement.classList.add('incorrect-login');
    errorLogin_message.innerText += 'Email is required.\n';
    valid = false;
  } else {
    loginEmail.parentElement.classList.remove('incorrect-login');
  }

  // Password validation
  if (loginPassword.value === '' || loginPassword.value === null) {
    loginPassword.parentElement.classList.add('incorrect-login');
    errorLogin_message.innerText += 'Password is required.\n';
    valid = false;
  } else {
    loginPassword.parentElement.classList.remove('incorrect-login');
  }
